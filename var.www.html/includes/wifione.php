<?php
    ...
    else if(isset($_GET['emac']))
    {
        ...
        $wpc = ($ssh->shell($qry));
		if(strstr($wpc,"{")==null){
			file_put_contents("/tmp/_web.err",$wpc);
			die();
		}
...
//            file_put_contents("/etc/wpa_supplicant/env", "WLAN={$iface}");
			$wlan=$_GET['w'];
            $ssh->exec("sudo /sbin/ifconfig {$wlan} down");
            $ssh->exec("sudo /sbin/ifconfig {$wlan} up");
            /*
			$ssh->exec("sudo kill -9 $(pidof dhclient)");
			$wlandhcp = shell_exec("grep wlan /etc/network/interfaces | grep dhcp");
			if(!empty($wlandhcp))
			{
            	$ssh->exec("sudo /sbin/dhclient {$wlan}");
			}
*/
            $ip =  $ssh->shell("/sbin/ifconfig {$wlan} | grep 'inet addr'| awk '{print $2}'");
            if(empty($ip))
            {
				file_put_contents("/tmp/_web.err","No IP on {$wlan}");
                die();
            }
			echo "<p id='interface_name' hidden>{$wlan}</p>";
            echo "Connected to {$ip}";
        }
        else{
			file_put_contents("/tmp/_web.err",$qry);
        }
        die();
    }
...
$(document).ready(function() {
   ...
   $('.hdefault').toggle();
    ...
    $(".wifipb").click(function() {
		...
		var interface = $('#interface_name').html();
        var pid = "p_" + emac;
        var wid = interface;
        var name = $(this).attr('name');
        var pass = $('#'+pid).val();
 		var uri = './includes/wifione.php?emac='+emac+'&name='+name+'&p='+pass+'&w='+wid;
        $.ajax({async: true,
                type: 'GET',
                url: uri,
                success: function(result)
        {
			$("#plw").show();
            $('#'+pbid).html("Connecting...");
            $('#' +'h_'+ emac).toggle(200);
			window.location.href="index.php?page=wifione&q="+uri;
...
<?php
	...
	{
		$ssh->shell("sudo touch /etc/wpa_supplicant/wpa_supplicant.conf");
		echo "config for wpa_suplicant is missing";
	}
    $ssh->exec("sudo systemctl restart wpa_supplicant");
...
{    
    ...
    // find on which we are 
	// connected
    $err= @file_get_contents("/tmp/_web.err");
	system("rm /tmp/_web.err");
    $alines = explode("\n",$ssh->shell("sudo /sbin/iwlist scanning"));
    $map = array();
    echo "<table width=800px' border='1' cellspacing='0' cellpading='0'>";
	echo "<tr><th colspan='4'><font color='red'>{$err}</font> <div id='interface_name'>{$wlan}</div></th></tr>";
    echo "<tr><th width='30%'>Network</th><th width='20%'>Signal</th><th width='20%'>Security</th><th width='30%'>Status</th></tr>";
    foreach ($alines as $l)
    {
        if(strstr($l,"Cell"))
        {
        ...
        if($enabled && $name && $qual!=-1)
        {
            ...
            else
            {
                ...
                echo "<td width='40%'><div class='wifikon' id='{$fid}' >Connect</div>";
                    ...
                    echo "<li><button class='btn btn-outline btn-primary btn-default wifipb' id='m_{$fid}' name='{$name}'>Connect...</button></div>";

...
function DisplayAuthConfig($username, $password)
{
  ...
  if (isset($_POST['UpdateAdminPassword'])) {
    if (1) {
		$up = file_get_contents("/etc/raspap/.password");
		$up=str_replace("\n","",$up);
		$upa=split(":",$up);
		var_dump($upa);
        if ($_POST['oldpass'] == $upa[1])
	    {
        ...
        if ($_POST['newpass'] !== $_POST['newpassagain']) {
          ...
          if ($auth_file = fopen(RASPI_ADMIN_DETAILS, 'w')) {
			...
			file_put_contents("/etc/raspap/.password",$username.":".$_POST['newpass']);
          } else {
    ...
    <div class="col-lg-12">
      <div class="panel panel-primary">
        ...
        <div class="panel-body">
          ...
          <form name="form" role="form" action="index.php?page=admin" method="POST">
            <?php CSRFToken() ?>
            <div class="row">
              <div class="form-group col-md-4">
                <label for="username"><?php echo _("Username"); ?></label>
                <input disabled type="text" class="form-control" name="username" value="admin"/>
              </div>
            </div>
            <div class="row">
              <div class="form-group col-md-4">
